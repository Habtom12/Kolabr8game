...
public class InputController extends MotionController implements KeyListener{
	...
	public InputController(PlayerTank player){
		this(player, new int[] {KeyEvent.VK_LEFT,KeyEvent.VK_UP, KeyEvent.VK_RIGHT, KeyEvent.VK_DOWN, KeyEvent.VK_SPACE});
		moveState = 0;
		this.player = true;
	}
	
	public InputController(PlayerTank player, int[] keys){
		this(player, keys, KoalaWorld.getInstance());

	}
	
	public InputController(PlayerTank player, int[] keys, Component world){
	...
	private void setMove(String direction) {
		try{
			field = PlayerTank.class.getDeclaredField(direction);
			moveState=1;
			this.setChanged();
	...
	private void setFire(){
		field = null;
		try{
			action = PlayerTank.class.getMethod("startFiring");
			this.setChanged();
	...
	private void unsetFire(){
		field = null;
		try{
			action = PlayerTank.class.getMethod("stopFiring");
			this.setChanged();
		} catch(NoSuchMethodException e){e.printStackTrace();}
		notifyObservers();
	}
	...
	public void read(Object theObject) {
		PlayerTank player = (PlayerTank) theObject;
