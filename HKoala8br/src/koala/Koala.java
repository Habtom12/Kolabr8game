...
public class Koala extends PlayerTank {
	int direction;
	...
	public Koala(Point location, Image img, int[] controls, String name) {
		...
		this.name = name;
		motion = new InputController(this, controls, KoalaGame.getInstance());
		height = 37;
		width = 37;
		direction = 0;
	...
	public void update(int w, int h) {	
		// allowed either left/right or up/down movement, not diagonal
	  	...
	  	if (right==1) {
	  		...
	  		img = KoalaGame.sprites.get("koala_right_1");
	  		
	  	} else 	if (left == 1) {
	  		location.x += (right-left) * 3; 
	  		img = KoalaGame.sprites.get("koala_left_1");

	  	}
	  	else if (down == 1){
    		location.y += (down-up) * 3;
	  		img = KoalaGame.sprites.get("koala_down_1");

    	}
	  	else if (up == 1) {
	  		location.y += (down-up) * 3;
	  		img = KoalaGame.sprites.get("koala_up_1");
	...
	public void draw(Graphics g, ImageObserver obs) {
		if (respawnCounter <= 0)
			...
			KoalaGame.getInstance().addClockObserver(this.motion);
			respawnCounter -= 1;
	...
	public void die() {
		...
		KoalaGame.getInstance().addBackground(explosion);
		lives -= 1;
		if (lives >= 0) {
			KoalaGame.getInstance().removeClockObserver(this.motion);
			reset();
		} else {
